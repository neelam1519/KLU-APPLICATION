...
class EncryptionService {
  ...
  encrypt.Key generateKeyFromUid(String email) {
  ...
  Future<Encrypted> encryptData(String email,String text) async {
    final aes = Encrypter(AES(generateKeyFromUid(email))); // Use the generated key for encryption
  ...
  Future<String> decryptData( String email, Encrypted encryptedData) async {
    final aes = Encrypter(AES(generateKeyFromUid(email))); // Use the generated key for decryption
  ...
  Future<String> getKmsKey() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('keyname: ${responseData.toString()}');
